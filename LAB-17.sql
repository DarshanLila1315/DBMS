-- LAB-17: Views exercises (solutions are provided as commented T-SQL statements)
-- Adjust object and column names if your schema uses different identifiers (e.g. PERSON vs EMPLOYEE).

-- Part – A:
-- 1. Create a view Employee_All with all columns.

CREATE VIEW Employee_All AS SELECT * FROM EMPLOYEE;
...
SELECT * FROM EMPLOYEE WHERE UPPER(DEPARTMENT) = 'SERVER';
...
-- 27. Create a view for the employees whose first name contains vowels.
 ...
 SELECT * FROM EMPLOYEE WHERE FIRSTNAME LIKE '%A%' OR FIRSTNAME LIKE '%E%' OR FIRSTNAME LIKE '%I%' OR FIRSTNAME LIKE '%O%' OR FIRSTNAME LIKE '%U%';
...
CREATE VIEW FourLetter_Name AS
 SELECT EID, FIRSTNAME, DEPARTMENT FROM EMPLOYEE WHERE LEN(FIRSTNAME) = 4;

-- 29. Create a view for the employees whose name starts with M and ends with N.

 CREATE VIEW Name_StartsM_EndsN AS
 SELECT * FROM EMPLOYEE WHERE FIRSTNAME LIKE 'M%' AND FIRSTNAME LIKE '%N';

-- 30. Create a view Transport_Dept that displays Transport department employees only

CREATE VIEW Transport_Dept AS
SELECT * FROM EMPLOYEE WHERE UPPER(DEPARTMENT) = 'TRANSPORT';