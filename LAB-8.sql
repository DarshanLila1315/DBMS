...
SELECT CITY, COUNT(*) AS Male_Employees FROM EMPLOYEE WHERE GENDER = 'Male' GROUP BY CITY;
...
SELECT CITY, DEPARTMENT, COUNT(*) FROM EMPLOYEE GROUP BY CITY, DEPARTMENT;
...
SELECT CITY, MAX(SALARY) AS Max_Salary, MIN(SALARY) AS Min_Salary FROM EMPLOYEE WHERE GENDER = 'Female' GROUP BY CITY;

-- 27. Calculate department, city, and gender wise average salary.
SELECT DEPARTMENT, CITY, GENDER, AVG(SALARY) AS Average_Salary FROM EMPLOYEE GROUP BY DEPARTMENT, CITY, GENDER;
...
SELECT DEPARTMENT, CITY, MAX(SALARY) FROM EMPLOYEE WHERE CITY LIKE 'R%' GROUP BY DEPARTMENT, CITY;